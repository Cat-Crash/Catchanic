...
func _on_dialogue_node_reposition_dialogue_area() -> void:
	...
	dialogueNode.playerPos = get_viewport().get_camera_3d().unproject_position(Vars.playerPosition)
...
func _input(event : InputEvent) -> void:
	if event.is_action_pressed('interact') and playerInRange and (Vars.currentGameState == Enums.GameState.OVERWORLD ):
		Vars.currentGameState = Enums.GameState.DIALOGUE
		dialogueNode.beginDialoguePath()

func _on_dialogue_node_end_of_dialogue() -> void:
	Vars.currentGameState = Enums.GameState.OVERWORLD
